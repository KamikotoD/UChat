<x-app-layout>
    <form method="POST" action="/boards/{{$board->id}}" class="max-w-lg mx-auto mt-8 p-6 bg-white shadow-lg rounded-lg">
        @csrf
        @method('PATCH')
        <x-slot name="header">
        ...
        <div class="mt-6">
            ...
            <div class="max-w-md mx-auto">
                <x-text-input id="title"
                              ...
                              type="text"
                              name="title"
                              value="{{$board->title}}" required autofocus autocomplete="title" />
            </div>
        ...
        <div class="flex items-center justify-between mt-6">
            <button type="submit" class="text-sm text-indigo-600 hover:text-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500" >
                {{ __('Update') }}
            </button>

            <x-primary-button class="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" href="/boards/{{ $board->id }}">
                {{ __('Cancel') }}
            </x-primary-button>
        </div>
    </form>
